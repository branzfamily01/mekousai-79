(() => {
  ...
  const dayNightItems = items
    .filter(item => item.groupType === '昼夜祭')
  ...
  dayNightItems.forEach(item => {
    ...
    [item.group, item.sourceGenre].filter(Boolean).forEach(text => {
    ...
    d.textContent = item.description || '昼夜祭参加団体です。';
    copy.appendChild(d);

    const place = document.createElement('p');
    place.className = 'program-place';
    place.textContent = '昼夜祭：体育館｜9/5 15:30–17:30（在校生限定）';
