(() => {
  ...
  function render() {
    ...
    if (!photos.length) {
      state.textContent = group ? `${groupLabel}の写真はまだありません。` : 'まだ写真はありません。写真が追加されると、ここに自動で並びます。';
      total.textContent = group ? `${groupLabel}｜0 PHOTOS` : '0 PHOTOS';
      return;
    }
    total.textContent = group ? `${groupLabel}｜${photos.length} PHOTOS` : `${photos.length} PHOTOS`;
    photos.forEach((photo, index) => {
      const figure = document.createElement('figure'); figure.className = 'gallery-card';
      const openButton = document.createElement('button'); openButton.type = 'button'; openButton.setAttribute('aria-label', `${index + 1}枚目の写真を拡大`);
      const img = document.createElement('img'); img.src = photo.url; img.alt = photo.caption || `${title}の写真 ${index + 1}`; img.loading = index < 4 ? 'eager' : 'lazy'; img.decoding = 'async';
      openButton.appendChild(img); openButton.addEventListener('click', () => open(index)); figure.appendChild(openButton);
      const caption = document.createElement('figcaption');
      if (photo.takenOn) { const time = document.createElement('time'); time.dateTime = photo.takenOn; time.textContent = fmtDate(photo.takenOn); caption.appendChild(time); }
      if (photo.caption) { const p = document.createElement('p'); p.textContent = photo.caption; caption.appendChild(p); }
      figure.appendChild(caption); grid.appendChild(figure);
    });
  }

  function showCurrent() {
    const photo = photos[current]; if (!photo) return;
    lightboxImage.src = photo.url; lightboxImage.alt = photo.caption || `${title}の写真 ${current + 1}`;
  ...
  lightbox.addEventListener('pointerdown', e => { pointerStart = e.clientX; });
  lightbox.addEventListener('pointerup', e => { if (pointerStart === null) return; const delta = e.clientX - pointerStart; pointerStart = null; if (Math.abs(delta) > 55) move(delta > 0 ? -1 : 1); });
  ...
  fetch(endpoint, { headers: { Accept:'application/json' } })
    ...
    .catch(() => { state.hidden = false; state.className = 'gallery-error'; state.textContent = '写真の読み込みに失敗しました。少し時間をおいて再読み込みしてください。'; total.textContent = '読み込みエラー'; });
})();
